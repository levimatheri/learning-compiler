...
public class LexerTests
{
    ...
    [Theory]
    [MemberData(nameof(TokenPairsWithSeparatorData))]
    ...
    public static TheoryData<SyntaxKind, string> TokensData
    {
        get
        {
            var data = new TheoryData<SyntaxKind, string>();
            foreach (var t in GetTokens().Concat(GetSeparators()))
                data.Add(t.kind, t.text);
            return data;
        }
    }

    public static TheoryData<SyntaxKind, string, SyntaxKind, string> TokenPairsData
    {
        get
        {
            var data = new TheoryData<SyntaxKind, string, SyntaxKind, string>();
            foreach (var t in GetTokenPairs())
                data.Add(t.t1Kind, t.t1Text, t.t2Kind, t.t2Text);
            return data;
        }
    }

    public static TheoryData<SyntaxKind, string, SyntaxKind, string, SyntaxKind, string> TokenPairsWithSeparatorData
    {
        get
        {
            var data = new TheoryData<SyntaxKind, string, SyntaxKind, string, SyntaxKind, string>();
            foreach (var t in GetTokenPairsWithSeparator())
                data.Add(t.t1Kind, t.t1Text, t.separatorKind, t.separatorText, t.t2Kind, t.t2Text);
            return data;
        }
    }
